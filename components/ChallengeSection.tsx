export default function ChallengeSection() {
    ...
    return (
        <section className="section-padding bg-charcoal-900 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-[600px] h-[400px] bg-clay-500/8 rounded-full blur-[120px] pointer-events-none" />
            <div className="section-container relative z-10">
                <div className="max-w-3xl">
                    <p className="text-clay-400 text-xs font-semibold uppercase tracking-[0.25em] mb-5">
                        The Problem
                    </p>
                    <h2 className="text-beige-50 mb-6" style={{ fontFamily: "var(--font-cormorant)", fontWeight: 300 }}>
                        You&apos;re running a business and a household. Something has to give.
                    </h2>
                    <p className="text-lg text-beige-200/80 leading-relaxed mb-16 max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {pains.map((pain, i) => (
                        <div
                            key={i}
                            className="group relative bg-espresso-800/30 rounded-2xl p-8 border border-white/10 hover:border-clay-500/40 transition-all duration-500"
                        >
                            <span
                                className="absolute top-8 right-8 text-5xl font-light text-white/15 select-none pointer-events-none"
                                style={{ fontFamily: "var(--font-cormorant)" }}
                            >
                                {pain.number}
                            </span>
                            <div className="w-10 h-px bg-clay-500 mb-6" />
                            <h3 className="text-xl font-semibold text-beige-50 mb-3 pr-12 leading-tight">
                                {pain.heading}
                            </h3>
                            <p className="text-beige-200/70 text-sm leading-relaxed">{pain.body}</p>
                        </div>
                    ))}
                </div>
            </div>
